<template>
  <div class="table-confirmed">
    <TableGlobal :orders="ordersConfrimed" :status="status" :fields="fields" :date-filter-orders="date" />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      // STORE ALL ORDERS
      ...mapState('allOrders', {
         ordersConfrimed: state => state.orders
        .filter(ele => ele.statussuivi === 'Confirmed')
        .map(ele => {
